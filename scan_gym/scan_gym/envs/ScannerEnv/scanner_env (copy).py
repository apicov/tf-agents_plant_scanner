...
class ScannerEnv(gym.Env):
    ...
    def __init__(self,inliers_ratios_file,setpoint=0.1):
        ...
        #[inliers ratio, distance from closest saved image, covered area (18 sections), number of actions executed since last save image action,
        ...
        highl = np.array([1.0,200,18,self.max_temp_moves_count,1.0,20,1])                                           
        ...
        #every zone is 40 images long (20 degress in this case)
        #1 means that there is already a kept pixture in that zone
        self.covered_zones = np.zeros(18)
        self.covered_area = 0
    ...
    def step(self, action):
        ...
        #3000
        #if self.num_steps >= 1500 or (self.temp_moves_count >=  self.max_temp_moves_count  and self.covered_area==18) :
    ...
    def get_area_section_n(self, current_position):
        return int(current_position/80)
